	...
	var TryloomUI = {
		...
		setDownloadLinks: function( imageUrl, filename ) {
			...
			if ( imageUrl ) {
				downloadBtn
					.attr( 'href', imageUrl )
					.attr( 'download', filename || 'try-on.png' )
					.show();
				downloadIcon
					.attr( 'href', imageUrl )
					.attr( 'download', filename || 'try-on.png' );
		...
		displayResultImage: function( imageUrl, filename, callbacks ) {
			...
			preloader.onerror = function( event ) {
				...
				console.error( 'Image load error details:', {
					url: imageUrl,
					status: preloader.naturalWidth === 0 ? 'failed' : 'unknown',
					complete: preloader.complete,
					event: event
				} );
				
				// Try direct fetch as fallback to get more error details
				if ( typeof fetch !== 'undefined' ) {
					...
					.then( function( blob ) {
						if ( blob.type.startsWith( 'image/' ) ) {
							// If fetch works, create object URL and use it
							var objectUrl = URL.createObjectURL( blob );
							self.currentObjectUrl = objectUrl; // Store for cleanup
							resultImg.attr( 'src', objectUrl );
							self.setDownloadLinks( imageUrl, filename ); // Keep original URL for download
							self.finishResultImageLoading();
							if ( callbacks.onLoad ) {
								callbacks.onLoad();
							}
					...
					// No fetch support, just show error
					self.finishResultImageLoading();
					self.setDownloadLinks( null );
		...
		uploadFile: function( file ) {
			...
			var progressHTML = '<div class="tryloom-upload-progress-overlay">' +
				'<div class="tryloom-progress-circle">' +
					'<svg width="80" height="80">' +
						'<circle class="tryloom-progress-circle-bg" cx="40" cy="40" r="36"></circle>' +
						'<circle class="tryloom-progress-circle-fill" cx="40" cy="40" r="36" ' +
						'stroke-dasharray="226" stroke-dashoffset="226"></circle>' +
					'</svg>' +
					'<span class="tryloom-progress-percent">0%</span>' +
				'</div>' +
			...
			formData.append( 'nonce', tryloom_params.nonce );
			formData.append( 'image', file );
			formData.append( 'save_photo', $( '#tryloom-save-photo' ).is( ':checked' ) ? 'yes' : 'no' );
		...
		generateTryOn: function() {
			...
			// Show loading overlay with 'Trying it on...'
			$( '.tryloom-loading-overlay' ).show();
			$( '.tryloom-loading-overlay p' ).text( 'Trying it on...' );
			...
			$.ajax( {
				...
				success: function( response ) {
					...
					if ( response.success ) {
						...
						setTimeout( function() {
							// Display result with spinner until load completes.
							TryloomUI.displayResultImage( response.data.image_url, response.data.filename, {
								onError: function() {
									var errorMsg = 'We could not load the generated image. ';
									errorMsg += 'Please check if you are logged in and try again. ';
									errorMsg += 'If the problem persists, please contact support.';
									TryloomUI.showErrorPopup( tryloom_params.i18n.error || errorMsg );
									TryloomUI.clearGenerationState();
									TryloomUI.showStep( 1 );
								}
							} );
						}, 500 ); // 500ms delay to ensure file is ready
					} else {
